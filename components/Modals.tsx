...
export const CreateModal: React.FC<CreateModalProps> = ({ onClose, type, initialData }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4">
        <div className="px-8 py-6 border-b flex justify-between items-center bg-indigo-50/50">
          <h2 className="text-2xl font-black text-indigo-900 uppercase tracking-tight">{type} Builder</h2>
        ...
        <div className="p-8 max-h-[75vh] overflow-y-auto no-scrollbar">
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input required className="w-full p-4 bg-gray-50 border rounded-2xl outline-none font-bold" value={title} onChange={(e) => setTitle(e.target.value)} />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Subject</label>
                <input required className="w-full p-4 bg-gray-50 border rounded-2xl outline-none font-bold" value={subject} onChange={(e) => setSubject(e.target.value)} />
              </div>
              {type === 'ASSIGNMENT' && (
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Due Date</label>
                  <input type="date" required className="w-full p-4 bg-gray-50 border rounded-2xl outline-none font-bold" value={deadline} onChange={(e) => setDeadline(e.target.value)} />
                </div>
              )}
            </div>

            {type === 'NOTE' && (
               <div className="border-2 border-dashed border-indigo-100 rounded-2xl p-6 text-center hover:bg-indigo-50/30 transition-all relative">
            ...
            <div>
              ...
              <textarea required className="w-full p-4 bg-gray-50 border rounded-2xl outline-none h-24 resize-none" value={description} onChange={(e) => setDescription(e.target.value)} />
            </div>

            <button disabled={loading} type="submit" className="w-full bg-gray-900 text-white py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-gray-800 disabled:opacity-50 flex items-center justify-center gap-3">
              {loading ? <Loader2 className="animate-spin" /> : <Save className="w-5 h-5" />}
              {initialData ? 'Save Changes' : `Publish ${type}`}
            </button>
          </form>
        </div>
      </div>
