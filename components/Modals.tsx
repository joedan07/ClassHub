...
export const SubmissionModal: React.FC<{ assignment: any, onClose: () => void }> = ({ assignment, onClose }) => {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Upload file to Supabase Storage Bucket named 'assignments'
      // Make sure you created this bucket in Supabase Dashboard -> Storage
      ...
      // 3. Create the permanent record in 'submissions' table
      console.log("Submitting with code:", context.classCode)
      const { error: dbError } = await supabase.from('submissions').insert({
        assignment_id: assignment.id,
        student_id: user.id,
        file_url: publicUrl,
        file_name: file.name,
        class_code: context.classCode // MUST NOT BE EMPTY
      });
...
// --- 2. THE CREATE MODAL (Revamped for Faculty logic) ---
