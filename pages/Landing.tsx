...
const Landing: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${isDarkMode ? 'bg-gray-950 text-gray-100' : 'bg-white text-gray-900'} selection:bg-indigo-500/30 selection:text-indigo-400`}>
      ...
      {/* NAVBAR - Completely isolated from the scale transform so it stays fixed! */}
      <nav className={`fixed top-0 w-full z-50 backdrop-blur-md border-b transition-all duration-500 ease-in-out ${isDarkMode ? 'bg-gray-950/80 border-gray-800' : 'bg-white/80 border-gray-200/50'} ${isExiting ? 'opacity-0 -translate-y-full' : 'opacity-100'}`}>
      ...
      {/* MAIN CONTENT - Gets the exit animation */}
      <main className={`transition-all duration-500 ease-in-out ${isExiting ? 'opacity-0 scale-[0.98]' : 'opacity-100 scale-100'}`}>
