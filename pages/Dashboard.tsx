...
import { Bell, Calendar, CheckCircle, Clock, ArrowRight, BookOpen, MapPin, X } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const [submittedIds, setSubmittedIds] = useState<string[]>([]); // Tracking real submissions
  ...
  useEffect(() => {
    const fetchData = async () => {
      // 1. Fetch User's Submissions to update tracker
  ...
  // Dynamic logic: Only count assignments NOT yet submitted and not expired
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-indigo-600 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <div className="text-indigo-200 font-medium">Pending Submissions</div>
          </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="font-bold text-gray-900 text-lg mb-1">Quick Status</div>
          <div className="text-gray-500 text-sm">
            {pendingAssignments === 0 ? "You're all caught up!" : `You have ${pendingAssignments} tasks to complete.`}
          </div>
        </div>
        ...
        <div onClick={() => setShowFullTimetable(true)} className="bg-gray-900 rounded-3xl p-6 text-white shadow-xl border border-gray-800 cursor-pointer hover:border-indigo-500 transition-colors group">
          <div className="flex justify-between items-start mb-2">
            ...
            <span className="text-[10px] uppercase tracking-widest bg-gray-800 px-2 py-1 rounded-md font-bold text-gray-300">Full Schedule ↗</span>
          </div>
          {nextClass ? (
            <div className="animate-in fade-in slide-in-from-bottom-2">
              <div className="text-xs font-black text-indigo-400 uppercase tracking-widest mb-1">Next Class</div>
              <div className="text-xl font-black text-white mb-1 line-clamp-1">{nextClass.subject_name}</div>
              <div className="text-sm font-medium text-gray-400 flex items-center gap-2"><Clock className="w-3.5 h-3.5"/> {formatTime(nextClass.start_time)} <span className="text-gray-600">|</span> <MapPin className="w-3.5 h-3.5"/> {nextClass.room_no || 'TBA'}</div>
            </div>
          ) : <div className="mt-4"><div className="text-xl font-black text-white mb-1">No more classes! 🎉</div><div className="text-sm font-medium text-gray-400">Rest up for tomorrow.</div></div>}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {assignments.filter(a => !submittedIds.includes(a.id)).slice(0, 3).map((a: any) => (
              <div key={a.id} className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between shadow-sm">
            ...
            {assignments.filter(a => !submittedIds.includes(a.id)).length === 0 && (
              <div className="bg-gray-50 rounded-3xl p-10 flex flex-col items-center justify-center text-center border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-500 text-sm">No pending submissions found in your records.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              ...
              <button onClick={() => setShowFullTimetable(false)} className="p-2 bg-white rounded-full text-gray-400 hover:text-gray-900 border border-gray-200"><X className="w-6 h-6" /></button>
            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {dayClasses.map(c => (
                        <div key={c.id} className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                          <div className="text-xs font-black text-indigo-600 mb-1">{formatTime(c.start_time)} - {formatTime(c.end_time)}</div>
                          <div className="font-bold text-gray-900 leading-tight mb-2">{c.subject_name}</div>
                          <div className="text-xs text-gray-500 font-medium">{c.faculty_name || 'TBA'}</div>
                          <div className="text-xs text-gray-500 font-medium mt-1 flex items-center gap-1"><MapPin className="w-3 h-3"/> {c.room_no || 'TBA'}</div>
                        </div>
                      ))}
                    </div>
                  </div>
